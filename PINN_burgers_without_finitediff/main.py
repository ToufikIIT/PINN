...
class Tanh:
    ...
    def backward(self, g_a, g_ax, g_at, g_axx):
        ddds = ddd_tanh(self.z)
        g_z = (g_a * self.ds) + \
              (g_ax * self.dds * self.z_x) + \
              (g_at * self.dds * self.z_t) + \
              (g_axx * (ddds * self.z_x**2 + self.dds * self.z_xx))
        g_zx = (g_ax * self.ds) + (g_axx * 2 * self.dds * self.z_x)
        g_zt = g_at * self.ds
        g_zxx = g_axx * self.ds
        return g_z, g_zx, g_zt, g_zxx
...
class Linear:
    ...
    def backward(self, g_z, g_zx, g_zt, g_zxx):
        dw = (g_z @ self.h.T) + (g_zx @ self.h_x.T) + (g_zt @ self.h_t.T) + (g_zxx @ self.h_xx.T)
        db = np.sum(g_z, axis=1, keepdims=True)
        return (dw, db), (self.W.T@g_z, self.W.T@g_zx, self.W.T@g_zt, self.W.T@g_zxx)
...
class PINN:
    ...
    def backward(self, g_u, g_ux, g_ut, g_uxx):
        grads = (g_u, g_ux, g_ut, g_uxx)
        l_grads = []
...
for epoch in range(epochs + 1):
    ...
    gf = (2/batch_size)*f
    grads_f = net.backward(gf*ux, gf*u, gf, -nu*gf)
